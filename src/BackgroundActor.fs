...
type private ConnectionString = string
type private Tags = string
type private ClubId = Guid
type private LinkId = Guid

type BackgroundMessages =
    | UpdateClubTags of ConnectionString * Tags * ClubId * LinkId
    | UpdateTags of string * string * Guid
    | PostSlack of PostToSlack
...
let updateLinkTagByName (dbCtx: DbProvider.Sql.dataContext) (name: string) (linkId: Guid) isCustom =
    async {
        updateTagByName dbCtx name |> Async.RunSynchronously |> ignore
...
let refreshLinkTags (dbCtx: DbProvider.Sql.dataContext) (tags: string) (linkId: Guid) isCustom =
    async {
            ...
            for lt in dbCtx.Public.LinkTags do
                for l in lt.``public.Links by Id`` do
                    for t in lt.``public.Tags by Name`` do
                        where (l.Id = linkId && lt.IsCustom = false)
                        select t
        }
        |> Seq.toArray
...
let updateClubTagByName (dbCtx: DbProvider.Sql.dataContext) (name: string) (clubId: Guid) (linkId: Guid) isCustom =
    async {
        updateTagByName dbCtx name |> Async.RunSynchronously |> ignore
        ...
        let! clubLinkTag =
            query {
                ...
                where (clubLinkTag.ClubId = clubId && clubLinkTag.LinkId = linkId && clubLinkTag.TagName = name)
                select clubLinkTag
            }
            |> Seq.tryHeadAsync
...
let refreshClubLinkTags (dbCtx: DbProvider.Sql.dataContext) (tags: string) (clubId: Guid) (linkId: Guid) isCustom =
    async {

        query {
            for clubLinkTag in dbCtx.Public.ClubLinkTags do
            for tag in clubLinkTag.``public.Tags by Name`` do
            where (clubLinkTag.ClubId = clubId && clubLinkTag.LinkId = linkId)
            select tag
        }
        |> Seq.toArray
        |> Seq.iter (fun t -> t.Count <- t.Count - 1L)
        ...
        query {
            for clubLinkTag in dbCtx.Public.ClubLinkTags do
            where (clubLinkTag.ClubId = clubId && clubLinkTag.LinkId = linkId && clubLinkTag.Count = 0L)
            select clubLinkTag
        }
        |> Seq.``delete all items from single table``
        |> Async.RunSynchronously
    ...
    MailboxProcessor.Start(fun inbox ->
        let rec processMessage () =
            async {
                ...
                let! _ =
                    match msg with
                    | UpdateTags (connString, tags, linkId) ->
                        let dbCtx = DbProvider.Sql.GetDataContext connString
                        refreshLinkTags dbCtx tags linkId false
                    | UpdateClubTags (connString, tags, clubId, linkId) ->
                        let dbCtx = DbProvider.Sql.GetDataContext connString
                        refreshClubLinkTags dbCtx tags clubId linkId true
                    | PostSlack (payload) ->
                        httpAsync {
                            POST "http://localhost:3500/v1.0/invoke/timon-slack/method/post-slack"
                            body
